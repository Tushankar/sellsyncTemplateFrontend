...
export const LoginSidebar: React.FC<LoginSidebarProps> = ({ variant = "ghost", className, triggerText }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (view === "login" && loginType === "builder") {
      // Mock cookie setting for local session bypass
      Cookies.set("authToken", "mocked-token", { expires: 1, path: "/" });
      Cookies.set("u_id", "mocked-user-id", { expires: 1, path: "/" });
      Cookies.set("u_type", "builder", { expires: 1, path: "/" });
                  ...
                  {(view === "login" || view === "signup" || view === "forgot") && (
                    <div className="space-y-2">
                      ...
                      <div className="relative">
                        ...
                        <input type="text" name="email" placeholder="name@example.com" className={`flex h-10 w-full rounded-md border bg-background px-3 py-2 pl-10 text-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 transition-all ${emailError ? "border-red-500 focus-visible:ring-red-500" : "border-input focus-visible:ring-orange-500"}`} value={email} onChange={handleOnChange} />
                      </div>
